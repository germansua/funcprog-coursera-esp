...
object TimeUsage {
    ...
    (
      ...
      columnNames.filter(columnName => {
        (columnName.startsWith("t02") || columnName.startsWith("t04") || columnName.startsWith("t06") ||
          ...
          columnName.startsWith("t18")) && !(columnName.startsWith("t1801") || columnName.startsWith("t1803") || columnName.startsWith("t1805"))
      }).map(columnName => col(columnName))
    )
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def roundValue(value: Double) = (value * 10d).round / 10d
    ...
    summed.groupByKey(timeUsageRow => (timeUsageRow.working, timeUsageRow.sex, timeUsageRow.age))
      ...
      .map {
        case ((working, sex, age), primaryNeeds, work, other) => TimeUsageRow(working, sex, age, roundValue(primaryNeeds), roundValue(work), roundValue(other))
      }
      .sort("working", "sex", "age")
  }
}
