 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of empty equeals zero") {
    assert(sum(List()) === 0)
  }

  test("sum of zeros equeals zero") {
    assert(sum(List(0, 0, 0, 0)) === 0)
  }

  test("sum of negatives is less than zero") {
    assert(sum(List(-1, -2, -3)) < 0)
  }

  test("sum of mirroring possitive and negative values equeals zero") {
    assert(sum(List(-1, -2, -3, 1, 2, 3)) === 0)
  }
  ...
  test("empty List should thorw an NoSuchElementException") {
  ...
  test("max of zero list values should be zero") {
