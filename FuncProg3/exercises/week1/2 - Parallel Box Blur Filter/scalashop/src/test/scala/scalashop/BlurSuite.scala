...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel of a 3x4 image with radius 3") {
    val src = new Img(3, 4)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8
    ...
    assert(boxBlurKernel(src, 1, 2, 3) === 9,
      s"(boxBlurKernel(1, 2, 1) should be 9, " +
        s"but it's ${boxBlurKernel(src, 1, 2, 1)})")
  }

  test("boxBlurKernel should return the correct value on the first pixel of a 3x4 image with radius 1") {
    val src = new Img(3, 4)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8
  ...
  test("boxBlurKernel should return the correct value on the last pixel of a 3x4 image with radius 2") {
    val src = new Img(3, 4)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8
    ...
    assert(boxBlurKernel(src, 2, 3, 2) === 12,
      s"(boxBlurKernel(1, 2, 1) should be 12, " +
        s"but it's ${boxBlurKernel(src, 2, 3, 2)})")
  }
...
//  test("VerticalBoxBlur.blur with radius 1 should correctly blur the strip 0 <= x < 2 of a 3x3 image") {
//    val w = 3
//    val h = 3
//    val src = new Img(w, h)
//    val dst = new Img(w, h)
...
//    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8;
//
//    VerticalBoxBlur.blur(src, dst, 0, 2, 2)
//
//    def check(x: Int, y: Int, expected: Int) =
//      assert(dst(x, y) == expected,
//        s"(destination($x, $y) should be $expected)")
...
//  }
  ...
  test("VerticalBoxBlur.parBlur with radius 1 should correctly blur the entire 4x3 image taken 1 and 2 columns") {
